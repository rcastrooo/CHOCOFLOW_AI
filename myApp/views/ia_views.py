...
def ia_predicciones(request):
    ...
    if request.method == 'POST':
        ...
        if tipo == 'produccion':
            ...
            top_carga = sorted(carga.items(), key=lambda x: x[1], reverse=True)[:3]

            # Turnos mañana vs tarde
            manana = asignaciones.filter(turno__horario='Mañana').count()
            tarde = asignaciones.filter(turno__horario='Tarde').count()

            # Promedio asignaciones por empleado
            ...
            lineas.append(f"\n🔮 Predicción próxima semana:")
            recomendado_manana = max(2, round(empleados_activos * 0.5))
            recomendado_tarde = max(2, round(empleados_activos * 0.5))
            lineas.append(f"  • Turno Mañana: se recomiendan {recomendado_manana} empleados")
            lineas.append(f"  • Turno Tarde: se recomiendan {recomendado_tarde} empleados")
